...
def spatial_temporal_mixup_3d(x, y, alpha=1.0, use_cuda=True):
...
def temporal_mixup_3d(x, y, allow_half=False, alpha=1.0, use_cuda=True):
    temporal_length = x.size()[3]
    if alpha > 0:
        if allow_half:
...
def temporal_mixup_2d(x, y, allow_half=False, alpha=1.0, use_cuda=True):
    temporal_length = x.size()[1]
    if alpha > 0:
        if allow_half:
            lam = np.random.randint(0, temporal_length // 2 + 1)
